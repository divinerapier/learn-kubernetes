...
readonly KUBE_GO_PACKAGE=github.com/divinerapier/learn-kubernetes
...
readonly KUBE_COVERAGE_INSTRUMENTED_PACKAGES=(
  github.com/divinerapier/learn-kubernetes/cmd/kube-apiserver
  github.com/divinerapier/learn-kubernetes/cmd/kube-controller-manager
  github.com/divinerapier/learn-kubernetes/cmd/kube-scheduler
  github.com/divinerapier/learn-kubernetes/cmd/kube-proxy
  github.com/divinerapier/learn-kubernetes/cmd/kubelet
)
...
# Argument: the name of a Kubernetes package (e.g. github.com/divinerapier/learn-kubernetes/cmd/kube-scheduler)
...
import (
  "testing"
  "github.com/divinerapier/learn-kubernetes/pkg/util/coverage"
)
    ...
    for package in "$@"; do
      if kube::golang::is_instrumented_package "${package}"; then
        ...
        go test -c -o "$(kube::golang::outfile_for_binary "${package}" "${platform}")" \
          -covermode count \
          -coverpkg k8s.io/...,github.com/divinerapier/learn-kubernetes/vendor/k8s.io/... \
          "${build_args[@]}" \
          -tags coverage \
          "${package}"
      else
